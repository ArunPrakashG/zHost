          ...
          switch (result.Status) {
            case "-1":
              Swal.fire(result.ShortReason, result.Reason, result.Level).then(
                (value) => {
                  document.location = "../Views/SendView.php";
                }
              );
              return;
            case "0":
    ...
    success: function (result) {
      switch (result.Status) {
        case "-1":
          Swal.fire(result.ShortReason, result.Reason, result.Level).then(
            (value) => {
              //document.location = "../Views/SendView.php?previousError=true";
              return;
            }
          );
          return;
...
function generateRowHtml(indexer, mail) {
  ...
  return (
    ...
    "</td>" +
    '<td class="table-row-field">' +
    '<button class="deletebttn" id="send-options" onclick="onDeleteAnchorClicked(this);">Delete</button>' +
    "</button>" +
    "</td>"
  );
}
...
function trashOptions(index) {
  Swal.fire({
    text: "You can Delete mail permanently.",
    icon: "info",
    showCancelButton: true,
